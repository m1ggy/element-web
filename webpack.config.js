...
if (!ogImageUrl)
  ogImageUrl = 'https://app.element.io/themes/element/img/logos/opengraph.png';
...
module.exports = (env, argv) => {
  ...
  function getThemesImports() {
    const imports = ACTIVE_THEMES.map((t, index) => {
      const themeImportPath = cssThemes[`theme-${t}`].replace(
        './node_modules/',
        ''
      );
    ...
    module: {
      ...
      rules: [
        ...
        },
        {
          ...
          options: {
            cacheDirectory: true,
            plugins: [useHMR && require.resolve('react-refresh/babel')].filter(
              Boolean
            ),
          },
        },
    ...
    plugins: [
      ...
      useHMR &&
        new ReactRefreshWebpackPlugin(
          fullPageErrors ? undefined : { overlay: { entry: false } }
        ),
    ].filter(Boolean),
